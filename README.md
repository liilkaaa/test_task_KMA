...
- Причини відмов (`decline_code`) переважають двома кодами (7322, 4670)
...
плюс SUM(IF [Transaction Type] IN ('REFUND','CHARGEBACK','VOID','ALERT','DISPUTE') THEN [Amount Usd] ELSE 0 END) 
...
Я використовувала ШІ інструменти (Claude та Gemini) для знаходження певних користувачів, у яких поведінка відрізняється від інших, щоб трохи зекономити час і не все шукати через SQL запити. Також використовувала для відшліфування фінального SQL запиту, якщо у мене виникали дивні результати або помилки. Також я знаходила метрики у статтях і потім у ШІ запитувала, які найкраще підійдуть до мого датасету. У EDA найбільше використовувала для візуалізацій, щоб це не займало багато часу. Ну і також ШІ допомагав мені писати деякі фрагменти коду. Також використала ШІ на фіналі, коли скинула всі свої рішення і висновки, і у результаті я мала все структуровано і все було готово для оформлення звіту
---
